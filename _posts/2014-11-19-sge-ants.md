...
Let's assume you run the main ants scripts like this

```bash
nohup rash run_ants.sh > ants_output.txt
```

You can now from any terminal type `tail -f ants_output.txt` to see how many jobs are pending and how many iterations are left. It is sometimes convenient to filter out the job numbers, we can do that with the following command

```bash
cat output.txt | grep -i "\(start\)\|\(wait\)" | less
```

For monitoring _SGE_ the first option is `qmon`. By clicking the _Queues control_ button you can see information about which clients are alive and how many jobs are they running. By clicking the _Job Control_ button you can see information about the pending and running jobs. From this screen you can also reschedule jobs which are hanged. The _qmon_ interface looks very old and has several usability problems. Sometimes is better to look at this information from the terminal. To see the list of running jobs use `qstat`, and to see the list of slaves use `qstat -f`. 
